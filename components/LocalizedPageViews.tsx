...
export function AboutPageView({ locale }: { locale: Locale }) {
  ...
  return (
    <SiteShell current="about" locale={locale} pathname={getRoute(locale, "about")}>
      <section className="mx-auto grid w-full max-w-[1760px] gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:px-10 lg:py-14">
        <div>
          ...
          <div className="mt-8 border-2 border-[#351815] bg-[#d96ab4] p-5 font-mono text-sm font-black uppercase leading-tight">
            {locale === "fr" ? "Ecole, metro, boulot dehors. Musique, sueur, lien social dedans." : "School, metro, work outside. Music, sweat, social link inside."}
          </div>
        </div>
        <div className="relative min-h-[520px] overflow-hidden border-2 border-[#351815] bg-[#351815]">
          <Image alt="NULLL.CLUB lifestyle mood" className="object-cover" fill sizes="(min-width: 1024px) 54vw, 100vw" src="/assets/nulll-new/water-face.png" />
      ...
      <section className="mx-auto grid w-full max-w-[1760px] gap-4 px-5 pb-14 sm:px-8 md:grid-cols-2 lg:px-10">
        {copy.aboutPage.values.map((value, index) => (
          <article className="border-2 border-[#351815] bg-[#f6eadf] p-6 transition hover:-translate-y-1 hover:shadow-[8px_8px_0_#ffb000] lg:p-8" key={value.title}>
            <p className="font-mono text-xs font-black uppercase text-[#d96ab4]">{String(index + 1).padStart(2, "0")} / {locale === "fr" ? "Valeur" : "Value"}</p>
...
export function CommunityPageView({ locale }: { locale: Locale }) {
  ...
  return (
    <SiteShell current="community" locale={locale} pathname={getRoute(locale, "community")}>
      <section className="mx-auto grid w-full max-w-[1760px] gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[1.02fr_0.98fr] lg:px-10 lg:py-14">
        <div>
          <SectionTitle as="h1" index="07" text={copy.communityPage.intro} title={copy.communityPage.title} />
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <div className="border-2 border-[#351815] bg-[#ffb000] p-4 font-mono text-xs font-black uppercase">No headphones</div>
            <div className="border-2 border-[#351815] bg-[#d96ab4] p-4 font-mono text-xs font-black uppercase">Come alone, leave connected</div>
          </div>
        </div>
        <div className="relative min-h-[560px] overflow-hidden border-2 border-[#351815] bg-[#351815]">
          <Image alt={locale === "fr" ? "Ambiance communautaire NULLL.CLUB" : "NULLL.CLUB community mood"} className="object-cover" fill priority sizes="(min-width: 1024px) 48vw, 100vw" src="/assets/nulll-new/smile-sun.png" />
        </div>
      </section>
      <section className="mx-auto grid w-full max-w-[1760px] gap-4 px-5 pb-14 sm:px-8 lg:grid-cols-3 lg:px-10">
