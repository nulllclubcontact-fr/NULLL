...
export function CheckoutForm({ locale }: { locale: Locale }) {
  ...
  if (status.type === "success") {
    return (
      <div className="border-2 border-[#773331] bg-[#F1EDE9] p-8" role="status">
        <p className="font-mono text-xs font-black uppercase text-[#EBA0CD]">Commande reçue</p>
        ...
        <div className="mt-8 flex flex-col gap-4 sm:flex-row">
          <Link className="inline-flex min-h-14 items-center border-2 border-[#773331] bg-[#773331] px-4 py-3 font-mono text-sm font-black uppercase text-[#F1EDE9]" href={getRoute(locale, "runs")}>
            Voir les prochains runs
          </Link>
          <Link className="inline-flex min-h-14 items-center border-2 border-[#773331] bg-[#F1EDE9] px-4 py-3 font-mono text-sm font-black uppercase text-[#773331]" href={getRoute(locale, "merch")}>
            Retour au merch
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.92fr_0.48fr]">
      <form className="border-2 border-[#773331] bg-[#F1EDE9] p-6 lg:p-8" onSubmit={handleSubmit}>
      ...
      <aside className="border-2 border-[#773331] bg-[#773331] p-6 text-[#F1EDE9]">
        <p className="font-mono text-xs font-black uppercase text-[#EBA0CD]">Récapitulatif</p>
        <div className="mt-4 space-y-4">
          {items.length ? (
            items.map((item) => (
              <div className="border-b-2 border-[#F1EDE9] pb-4" key={item!.id}>
